...
U = 6.0
site_num = 23
dos_data_name = "data/dos/Phase323_t=-1.00_U={0:.2f}.npz".format(U)
...
for index in range(site_num):
    ...
    print("Electron number on {0:2d}th site: {1:8f}".format(index, num))

fig_dos0, axes_dos0 = plt.subplots(2, 5, sharex="all", sharey="all")
fig_dos1, axes_dos1 = plt.subplots(2, 5, sharex="all", sharey="all")
fig_dos2, axes_dos2 = plt.subplots(1, 3, sharex="all", sharey="all")
for index in range(site_num):
    if index < 10:
...
ax_dos_avg.grid(ls="dashed", color="gray", axis="both")
