...
from DataBase import POINTS13, POINTS14, VECTORS
from TightBinding import TightBinding, TypicalSolver


cluster = Lattice(POINTS13["(6,7)"], VECTORS)
M = cluster.bs[0] / 2
...
GKMGEs = TightBinding(kpoints, cluster, return_vectors=False, t0=t0, t1=t1)
...
for index in range(13):
    ...
    axes_DOS[row, col].plot(projected_dos[:, index], omegas, lw=1.0)
    axes_DOS[row, col].axhline(mu, ls="dashed", color="red", lw=0.5)
...
axes_DOS[1, 6].plot(total_dos / 13, omegas, lw=1.0)
axes_DOS[1, 6].axhline(mu, ls="dashed", color="red", lw=0.5)
axes_DOS[1, 6].set_title("total")
axes_DOS[1, 6].grid(True, ls="dashed", color="gray")
fig_DOS.set_size_inches(9.6, 4.27)
plt.show()
fig_DOS.savefig("DOS_" + params_identity + ".png", dpi=500)
plt.close("all")
